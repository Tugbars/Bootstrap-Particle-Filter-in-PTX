 ...
 * @brief SMC² with RBPF Inner Filter - Kernel Implementations (8-param version)
 *
 * Learned parameters (8D):
 *   rho, sigma_total, r_split, mu_base, mu_scale, mu_rate, sigma_scale,
 * sigma_rate Derived: sigma_z = r_split * sigma_total,  sigma_base = sqrt(1-r²)
 * * sigma_total Fixed: θ(z) speed curve (θ_base, θ_scale, θ_rate) in constant
 * memory
 *
...
__device__ void ocsn_kalman_update(float y, float mu_pred, float var_pred,
 ...
 *
 * Order: rho, sigma_total, r_split, mu_base, mu_scale, mu_rate, sigma_scale,
 * sigma_rate
 *═══════════════════════════════════════════════════════════════════════════════*/

__device__ float log_prior_theta(float rho, float sigma_total, float r_split,
                                 float mu_base, float mu_scale, float mu_rate,
                                 float sigma_scale, float sigma_rate) {
  if (rho < d_bounds.rho_min || rho > d_bounds.rho_max)
    return -INFINITY;
  if (sigma_total < d_bounds.sigma_total_min ||
      sigma_total > d_bounds.sigma_total_max)
    return -INFINITY;
  if (r_split < d_bounds.r_split_min || r_split > d_bounds.r_split_max)
    return -INFINITY;
  if (mu_base < d_bounds.mu_base_min || mu_base > d_bounds.mu_base_max)
    return -INFINITY;
  if (mu_scale < d_bounds.mu_scale_min || mu_scale > d_bounds.mu_scale_max)
    return -INFINITY;
  if (mu_rate < d_bounds.mu_rate_min || mu_rate > d_bounds.mu_rate_max)
    return -INFINITY;
  if (sigma_scale < d_bounds.sigma_scale_min ||
      sigma_scale > d_bounds.sigma_scale_max)
    return -INFINITY;
  if (sigma_rate < d_bounds.sigma_rate_min ||
      sigma_rate > d_bounds.sigma_rate_max)
    return -INFINITY;

  float d_rho = (rho - d_prior.rho_mean) / d_prior.rho_std;
  float d_st =
      (sigma_total - d_prior.sigma_total_mean) / d_prior.sigma_total_std;
  ...
  float d_ss =
      (sigma_scale - d_prior.sigma_scale_mean) / d_prior.sigma_scale_std;
...
__device__ __forceinline__ int64_t z_noise_slot(int theta_idx, int t,
                                                int inner_idx, int N_inner,
                                                int cap) {
  int t_slot = t % cap;
  return (int64_t)theta_idx * N_inner * cap + (int64_t)t_slot * N_inner +
         inner_idx;
}

__device__ __forceinline__ int64_t u0_noise_slot(int theta_idx, int t,
                                                 int cap) {
  int t_slot = t % cap;
...
__global__ void kernel_init_rng(curandState *states, unsigned long long seed,
                                int N) {
...
__global__ void kernel_init_from_prior(ThetaParticlesSoA particles, int N_theta,
                                       int N_inner, noise_t *d_z_noise,
                                       noise_t *d_u0_noise,
                                       int noise_capacity) {
  ...
  __shared__ float s_sigma_z,
      s_sigma_base; /* derived from (sigma_total, r_split) */

  if (inner_idx == 0) {
    int attempts = 0, valid = 0;
    while (!valid && attempts < 1000) {
      s_rho = d_prior.rho_mean + d_prior.rho_std * curand_normal(rng);
      s_sigma_total = d_prior.sigma_total_mean +
                      d_prior.sigma_total_std * curand_normal(rng);
      s_r_split =
          d_prior.r_split_mean + d_prior.r_split_std * curand_normal(rng);
      s_mu_base =
          d_prior.mu_base_mean + d_prior.mu_base_std * curand_normal(rng);
      s_mu_scale =
          d_prior.mu_scale_mean + d_prior.mu_scale_std * curand_normal(rng);
      s_mu_rate =
          d_prior.mu_rate_mean + d_prior.mu_rate_std * curand_normal(rng);
      s_sigma_scale = d_prior.sigma_scale_mean +
                      d_prior.sigma_scale_std * curand_normal(rng);
      s_sigma_rate =
          d_prior.sigma_rate_mean + d_prior.sigma_rate_std * curand_normal(rng);

      valid = (s_rho >= d_bounds.rho_min && s_rho <= d_bounds.rho_max &&
               s_sigma_total >= d_bounds.sigma_total_min &&
               s_sigma_total <= d_bounds.sigma_total_max &&
               s_r_split >= d_bounds.r_split_min &&
               s_r_split <= d_bounds.r_split_max &&
               s_mu_base >= d_bounds.mu_base_min &&
               s_mu_base <= d_bounds.mu_base_max &&
               s_mu_scale >= d_bounds.mu_scale_min &&
               s_mu_scale <= d_bounds.mu_scale_max &&
               s_mu_rate >= d_bounds.mu_rate_min &&
               s_mu_rate <= d_bounds.mu_rate_max &&
               s_sigma_scale >= d_bounds.sigma_scale_min &&
               s_sigma_scale <= d_bounds.sigma_scale_max &&
               s_sigma_rate >= d_bounds.sigma_rate_min &&
               s_sigma_rate <= d_bounds.sigma_rate_max);
      attempts++;
    }
    ...
    s_sigma_base =
        sqrtf(fmaxf(1.0f - s_r_split * s_r_split, 1e-6f)) * s_sigma_total;
  ...
  int64_t z_noise_idx =
      z_noise_slot(theta_idx, 0, inner_idx, N_inner, noise_capacity);
  float z_noise_init =
      noise_store_roundtrip(d_z_noise, z_noise_idx, z_noise_raw);
  ...
  float theta_z = eval_curve(d_theta_curve.base, d_theta_curve.scale,
                             d_theta_curve.rate, z);
...
__global__ __launch_bounds__(N_INNER) void kernel_rbpf_step_impl(
    ThetaParticlesSoA particles, float y_obs, int N_theta, noise_t *d_z_noise,
    noise_t *d_u0_noise, int t_current, int noise_capacity) {
  ...
  int64_t z_noise_idx = z_noise_slot(theta_idx, t_current + 1, inner_idx,
                                     N_INNER, noise_capacity);
  int64_t u0_noise_idx =
      u0_noise_slot(theta_idx, t_current + 1, noise_capacity);
  ...
  if (inner_idx == 0) {
    ...
    float u0_stored =
        noise_store_roundtrip(d_u0_noise, u0_noise_idx, u0_noise_raw);
  ...
  float ll_incr =
      s_log_max + __logf(fmaxf(s_sum_w, 1e-30f)) - __logf((float)N_INNER);
...
__global__ void kernel_reset_outer_weights(ThetaParticlesSoA particles,
                                           int N_theta) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx >= N_theta)
    return;
...
/*═══════════════════════════════════════════════════════════════════════════════
 * KERNEL: Compute Particle Moments (8D)
 *
 ...
 */
__global__ void kernel_compute_particle_moments(ThetaParticlesSoA particles,
  ...
  param_ptrs[2] = particles.r_split;
  param_ptrs[3] = particles.mu_base;
  param_ptrs[4] = particles.mu_scale;
  param_ptrs[5] = particles.mu_rate;
  param_ptrs[6] = particles.sigma_scale;
  param_ptrs[7] = particles.sigma_rate;
...
__global__ void kernel_compute_outer_ess(ThetaParticlesSoA particles,
                                         float *d_ess_out, int N_theta) {
  extern __shared__ float s_data[];
  int tid = threadIdx.x;
...
__global__ void kernel_outer_resample(ThetaParticlesSoA particles,
                                      int *d_ancestors, float *d_uniform,
                                      int N_theta) {
  ...
  /* Serial prefix sum in thread 0 — fine up to shared mem limit (~12K
   * particles) */
  if (tid == 0) {
...
__global__ void kernel_copy_theta_particles(ThetaParticlesSoA src,
                                            ThetaParticlesSoA dst,
                                            int *d_ancestors, int N_theta,
                                            int N_inner,
  ...
  if (inner_idx < N_inner) {
    ...
    curand_init(resample_seed, (unsigned long long)dst_idx, 0,
                &dst.rng_states[dst_idx]);
  }
}
...
__global__ void
kernel_copy_noise_arrays(const noise_t *src_z_noise, noise_t *dst_z_noise,
  ...
  for (int t = t_start; t <= t_current + 1; t++) {
    int64_t src_idx =
        z_noise_slot(ancestor, t, inner_idx, N_inner, noise_capacity);
    int64_t dst_idx =
        z_noise_slot(theta_idx, t, inner_idx, N_inner, noise_capacity);
...
__global__ void
kernel_copy_checkpoint(const float *src_z, const float *src_mu_h,
                       const float *src_var_h, const float *src_log_w,
                       const float *src_ll, float *dst_z, float *dst_mu_h,
                       float *dst_var_h, float *dst_log_w, float *dst_ll,
                       const int *d_ancestors, int N_theta, int N_inner) {
  int theta_idx = blockIdx.x;
  int inner_idx = threadIdx.x;
  if (theta_idx >= N_theta || inner_idx >= N_inner)
...
/*═══════════════════════════════════════════════════════════════════════════════
 * KERNEL: CPMMH Fused Rejuvenation (8D proposal)
 *
...
template <int N_INNER>
__global__ __launch_bounds__(N_INNER) void kernel_cpmmh_rejuvenate_fused_impl(
    ThetaParticlesSoA particles, ThetaParticlesSoA particles_scratch,
    const float *y_history, noise_t *d_z_noise_curr, noise_t *d_z_noise_other,
    noise_t *d_u0_noise_curr, noise_t *d_u0_noise_other, int t_current,
    int N_theta, int noise_capacity, float cpmmh_rho, int *d_accepts,
    int *d_swap_flags, int t_checkpoint, const float *d_checkpoint_z,
    const float *d_checkpoint_mu_h, const float *d_checkpoint_var_h,
    const float *d_checkpoint_log_w, const float *d_checkpoint_ll) {
  static_assert(N_INNER <= 1024, "N_INNER must be <= 1024");
  ...
  if (inner_idx == 0) {
    ...
    for (int i = 0; i < N_PARAMS; i++)
      z_rnd[i] = curand_normal(&local_rng);
    ...
    s_sigma_base_prop =
        sqrtf(fmaxf(1.0f - s_r_split_prop * s_r_split_prop, 1e-6f)) *
        s_sigma_total_prop;
  ...
  if (t_checkpoint >= 0 && d_checkpoint_z != nullptr) {
    ...
    float theta_z_init = eval_curve(d_theta_curve.base, d_theta_curve.scale,
                                    d_theta_curve.rate, z_init);
    float mu_z_init = eval_curve(mu_base, mu_scale, mu_rate, z_init);
    float sigma_h_init =
        eval_curve(sigma_base, sigma_scale, sigma_rate, z_init);
    float phi_init = 1.0f - theta_z_init;
    float h_stat_var = (sigma_h_init * sigma_h_init) /
                       fmaxf(1.0f - phi_init * phi_init, 1e-6f);
  ...
  for (int t = t_start; t <= t_current; t++) {
    ...
    int64_t z_idx_t1 =
        z_noise_slot(theta_idx, t + 1, inner_idx, N_INNER, noise_capacity);
    float z_noise_curr_t1 = noise_load(d_z_noise_curr, z_idx_t1);
    float z_noise_fresh_t1 = curand_normal(&local_rng);
    float z_noise_prop_t1_raw =
        cpmmh_rho * z_noise_curr_t1 + scale * z_noise_fresh_t1;
    float z_noise_prop_t1 =
        noise_store_roundtrip(d_z_noise_other, z_idx_t1, z_noise_prop_t1_raw);
    ...
    if (inner_idx == 0) {
      ...
      float u0_noise_prop_raw =
          cpmmh_rho * u0_noise_curr + scale * u0_noise_fresh;
      float u0_stored =
          noise_store_roundtrip(d_u0_noise_other, u0_idx_t1, u0_noise_prop_raw);
    ...
    /* Propagate + Kalman with proposed curve params */
    ...
    float ll_incr =
        log_max + __logf(fmaxf(sum_w, 1e-30f)) - __logf((float)N_INNER);
    ll_accum += ll_incr;
  ...
  if (inner_idx == 0) {
    ...
    float log_alpha =
        (ll_prop_effective + s_lp_prop) - (ll_curr_effective + s_lp_curr);
...
__global__ void
kernel_commit_accepted_noise(noise_t *d_z_noise_0, noise_t *d_z_noise_1,
...
__global__ void
kernel_save_checkpoint(const ThetaParticlesSoA particles, float *d_checkpoint_z,
                       float *d_checkpoint_mu_h, float *d_checkpoint_var_h,
                       float *d_checkpoint_log_w, float *d_checkpoint_ll,
...
SMC2StateCUDA *smc2_cuda_alloc(int N_theta, int N_inner) {
  ...
  if (N_inner != 64 && N_inner != 128 && N_inner != 256 && N_inner != 512) {
    fprintf(stderr,
            "ERROR: N_inner=%d not supported. Use 64, 128, 256, or 512.\n",
            N_inner);
    exit(EXIT_FAILURE);
  }
  ...
  state->K_rejuv = 15; /* 8D space needs more moves per rejuvenation */

  int N_total = N_theta * N_inner;

  /* 8 learned θ-level params */
  ALLOC_THETA_FIELD(rho);
  ALLOC_THETA_FIELD(sigma_total);
  ALLOC_THETA_FIELD(r_split);
  ALLOC_THETA_FIELD(mu_base);
  ALLOC_THETA_FIELD(mu_scale);
  ALLOC_THETA_FIELD(mu_rate);
  ALLOC_THETA_FIELD(sigma_scale);
  ALLOC_THETA_FIELD(sigma_rate);
  ...
  CUDA_CHECK(cudaMalloc(&state->d_particles.rng_states,
                        N_total * sizeof(curandState)));
  CUDA_CHECK(cudaMalloc(&state->d_particles_temp.rng_states,
                        N_total * sizeof(curandState)));

  /* RNG init */
  kernel_init_rng<<<(N_total + 255) / 256, 256>>>(state->d_particles.rng_states,
                                                  12345ULL, N_total);
  CUDA_CHECK(cudaDeviceSynchronize());

  /* Observation history */
  state->y_history_capacity = 8000;
  CUDA_CHECK(cudaMalloc(&state->d_y_history,
   ...
   * This gets right-sized when set_fixed_lag is called. */
  {
    ...
    int default_cap =
        (int)(512LL * 1024 * 1024 / (per_slot_bytes > 0 ? per_slot_bytes : 1));
    if (default_cap < 64)
      default_cap = 64;
    if (default_cap > 2048)
      default_cap = 2048;
  ...
  CUDA_CHECK(
      cudaMalloc(&state->d_u0_noise[0], noise_array_bytes(u0_noise_size)));
  CUDA_CHECK(
      cudaMalloc(&state->d_u0_noise[1], noise_array_bytes(u0_noise_size)));

  /* Prior (8 params): ρ, σ_total, r_split, μ_base, μ_scale, μ_rate, σ_scale,
   * σ_rate σ_total ≈ √(σ_z² + σ_base²), r = σ_z/σ_total */
  state->prior.rho_mean = 0.95f;
  state->prior.rho_std = 0.02f;
  state->prior.sigma_total_mean = 0.18f;
  state->prior.sigma_total_std = 0.1f;
  state->prior.r_split_mean = 0.5f;
  state->prior.r_split_std = 0.2f;
  ...
  state->bounds.sigma_total_min = 0.01f;
  state->bounds.sigma_total_max = 1.5f;
  state->bounds.r_split_min = 0.01f;
  state->bounds.r_split_max = 0.99f;
  state->bounds.mu_base_min = -10.0f;
  state->bounds.mu_base_max = 5.0f;
  state->bounds.mu_scale_min = 0.01f;
  state->bounds.mu_scale_max = 5.0f;
  state->bounds.mu_rate_min = 0.1f;
  state->bounds.mu_rate_max = 10.0f;
  state->bounds.sigma_scale_min = 0.001f;
  state->bounds.sigma_scale_max = 1.0f;
  state->bounds.sigma_rate_min = 0.1f;
  state->bounds.sigma_rate_max = 10.0f;

  /* Theta curve (fixed — derived from φ-based sufficient stats) */
  state->theta_curve.base = 0.02f;
  state->theta_curve.scale = 0.08f;
  state->theta_curve.rate = 1.5f;

  /* Proposal std (8 params) */
  state->proposal_std[0] = 0.01f; /* rho */
  state->proposal_std[1] = 0.02f; /* sigma_total */
  state->proposal_std[2] = 0.05f; /* r_split — weakly identified */
  state->proposal_std[3] = 0.1f;  /* mu_base */
  state->proposal_std[4] = 0.05f; /* mu_scale */
  ...
  CUDA_CHECK(
      cudaMalloc(&state->d_checkpoint_scratch_z, N_total * sizeof(float)));
  CUDA_CHECK(
      cudaMalloc(&state->d_checkpoint_scratch_mu_h, N_total * sizeof(float)));
  CUDA_CHECK(
      cudaMalloc(&state->d_checkpoint_scratch_var_h, N_total * sizeof(float)));
  CUDA_CHECK(
      cudaMalloc(&state->d_checkpoint_scratch_log_w, N_total * sizeof(float)));
  CUDA_CHECK(
      cudaMalloc(&state->d_checkpoint_scratch_ll, N_theta * sizeof(float)));

  /* Adaptive proposal scratch (8D) */
  CUDA_CHECK(cudaMalloc(&state->d_temp_mean, N_PARAMS * sizeof(float)));
  CUDA_CHECK(
      cudaMalloc(&state->d_temp_cov, N_PARAMS * N_PARAMS * sizeof(float)));
...
void smc2_cuda_free(SMC2StateCUDA *state) {
  ...
  FREE_THETA_FIELD(mu_scale);
  FREE_THETA_FIELD(mu_rate);
  FREE_THETA_FIELD(sigma_scale);
  FREE_THETA_FIELD(sigma_rate);
...
void smc2_cuda_set_noise_capacity(SMC2StateCUDA *state, int capacity) {
  ...
  if (capacity <= state->noise_capacity)
    return;

  if (capacity > MAX_NOISE_CAPACITY) {
    fprintf(
        stderr,
        "WARNING: Requested noise_capacity=%d exceeds MAX_NOISE_CAPACITY=%d. "
        "Clamping to %d.\n",
  ...
  int64_t old_z_size =
      (int64_t)state->N_theta * state->N_inner * state->noise_capacity;
  ...
  int64_t total_bytes =
      4 * noise_array_bytes(new_z_size) + 4 * noise_array_bytes(new_u0_size);
  if (total_bytes > (int64_t)8 * 1024 * 1024 * 1024LL) {
    static bool warned = false;
    if (!warned) {
      fprintf(stderr,
              "WARNING: Noise buffer for %d slots would need %.1f GB "
              "(N_theta=%d, N_inner=%d). Using L=0 at this particle count "
              "requires more VRAM — use set_fixed_lag(L>0) to bound memory.\n",
              capacity, (double)total_bytes / (1024.0 * 1024.0 * 1024.0),
   ...
   * positions when capacity changes. But this is only called before
   * init_from_prior (which zeroes everything), so a raw copy of the smaller of
   * old/new is safe. */
  if (state->d_z_noise[0] && old_z_size > 0) {
    ...
    CUDA_CHECK(cudaMemcpy(new_z_0, state->d_z_noise[0],
                          noise_array_bytes(copy_z), cudaMemcpyDeviceToDevice));
  }
...
void smc2_cuda_set_fixed_lag(SMC2StateCUDA *state, int L) {
  ...
  if (L > 0) {
    ...
    if (state->noise_capacity != target_cap) {
      int64_t new_z_size =
          (int64_t)state->N_theta * state->N_inner * target_cap;
      int64_t new_u0_size = (int64_t)state->N_theta * target_cap;

      /* Sanity check: shouldn't exceed 8 GB */
      int64_t total_bytes = 4 * noise_array_bytes(new_z_size) +
                            4 * noise_array_bytes(new_u0_size);
      if (total_bytes > (int64_t)8 * 1024 * 1024 * 1024LL) {
        fprintf(stderr,
                "WARNING: Fixed-lag noise allocation would require %.1f GB. "
...
void smc2_cuda_set_proposal_std(SMC2StateCUDA *state, const float *std) {
  if (std) {
    ...
    state->proposal_std[0] = 0.01f; /* rho */
    state->proposal_std[1] = 0.02f; /* sigma_total */
    state->proposal_std[2] = 0.05f; /* r_split */
    state->proposal_std[3] = 0.1f;  /* mu_base */
    state->proposal_std[4] = 0.05f; /* mu_scale */
    state->proposal_std[5] = 0.1f;  /* mu_rate */
    state->proposal_std[6] = 0.02f; /* sigma_scale */
  ...
  CUDA_CHECK(cudaMemcpyToSymbol(d_proposal_std, state->proposal_std,
...
void smc2_update_adaptive_covariance(SMC2StateCUDA *state) {
  if (!state->use_adaptive_proposals)
    return;
  ...
  kernel_compute_particle_moments<<<1, block_size,
                                    block_size * sizeof(float)>>>(
      state->d_particles, state->d_temp_mean, state->d_temp_cov,
      state->N_theta);
  CUDA_CHECK(cudaDeviceSynchronize());

  CUDA_CHECK(cudaMemcpy(h_cov, state->d_temp_cov,
                        N_PARAMS * N_PARAMS * sizeof(float),
                        cudaMemcpyDeviceToHost));

  /* Regularize */
  for (int i = 0; i < N_PARAMS; i++)
    h_cov[i * N_PARAMS + i] += 1e-6f;
  ...
  for (int i = 0; i < N_PARAMS; i++) {
    for (int j = 0; j <= i; j++) {
      float sum = 0.0f;
      for (int k = 0; k < j; k++)
        sum += h_chol[i * N_PARAMS + k] * h_chol[j * N_PARAMS + k];
      if (i == j) {
        ...
        h_chol[i * N_PARAMS + j] =
            (diag > 1e-10f) ? (h_cov[i * N_PARAMS + j] - sum) / diag : 0.0f;
      }
    }
  }
  ...
  CUDA_CHECK(cudaMemcpyToSymbol(d_proposal_chol, h_chol,
                                N_PARAMS * N_PARAMS * sizeof(float)));
}
...
void smc2_cuda_init_from_prior(SMC2StateCUDA *state) {
  ...
  CUDA_CHECK(
      cudaMemcpyToSymbol(d_theta_curve, &state->theta_curve, sizeof(SVCurve)));
  CUDA_CHECK(cudaMemcpyToSymbol(d_proposal_std, state->proposal_std,
                                N_PARAMS * sizeof(float)));
  ...
  unsigned long long rng_seed =
      (state->user_seed != 0) ? state->user_seed : 12345ULL;
  kernel_init_rng<<<(N_total + 255) / 256, 256>>>(state->d_particles.rng_states,
                                                  rng_seed, N_total);
  CUDA_CHECK(cudaDeviceSynchronize());
...
float smc2_cuda_update(SMC2StateCUDA *state, float y_obs) {
  ...
  if (state->y_history_len >= state->y_history_capacity) {
    ...
    CUDA_CHECK(cudaMemcpy(new_hist, state->d_y_history,
                          state->y_history_len * sizeof(float),
                          cudaMemcpyDeviceToDevice));
    cudaFree(state->d_y_history);
  ...
  if (state->fixed_lag_L == 0 && state->t_current >= state->noise_capacity) {
    ...
    if (!growth_failed) {
      ...
      if (state->noise_capacity == old_cap) {
        growth_failed = true;
        fprintf(stderr,
                "NOTE: L=0 CPMMH will use wrapped noise from t=%d onward. "
...
#define DISPATCH_RBPF_STEP(N)                                                  \
  ...
  case 64:
    DISPATCH_RBPF_STEP(64);
    break;
  case 128:
    DISPATCH_RBPF_STEP(128);
    break;
  case 256:
    DISPATCH_RBPF_STEP(256);
    break;
  case 512:
    DISPATCH_RBPF_STEP(512);
    break;
  default:
...
#undef DISPATCH_RBPF_STEP
  ...
  float h_ess;
  CUDA_CHECK(
      cudaMemcpy(&h_ess, state->d_ess, sizeof(float), cudaMemcpyDeviceToHost));

  if (h_ess < state->ess_threshold_outer * state->N_theta) {
    state->n_resamples++;

    float h_uniform = xorshift64star_uniform(&state->host_rng_state);
    CUDA_CHECK(cudaMemcpy(state->d_uniform, &h_uniform, sizeof(float),
                          cudaMemcpyHostToDevice));

    int resample_block = (state->N_theta < 1024) ? state->N_theta : 1024;
    kernel_outer_resample<<<1, resample_block,
                            state->N_theta * sizeof(float)>>>(
        state->d_particles, state->d_ancestors, state->d_uniform,
        state->N_theta);
    /* No sync — next kernel depends on d_ancestors, same stream */

    unsigned long long resample_seed =
        time(NULL) * 1000ULL + state->n_resamples * 12345ULL;
    ...
    kernel_copy_noise_arrays<<<state->N_theta, state->N_inner>>>(
        ...
        state->d_ancestors, state->N_theta, state->N_inner, state->t_current,
        state->noise_capacity, t_noise_start);
    ...
    if (state->fixed_lag_L > 0 && state->t_checkpoint >= 0) {
      kernel_copy_checkpoint<<<state->N_theta, state->N_inner>>>(
          ...
          state->d_checkpoint_var_h, state->d_checkpoint_log_w,
          state->d_checkpoint_ll, state->d_checkpoint_scratch_z,
          state->d_checkpoint_scratch_mu_h, state->d_checkpoint_scratch_var_h,
          state->d_checkpoint_scratch_log_w, state->d_checkpoint_scratch_ll,
          state->d_ancestors, state->N_theta, state->N_inner);
      CUDA_CHECK(cudaDeviceSynchronize());

      /* Swap scratch → checkpoint */
      int N_total = state->N_theta * state->N_inner;
      CUDA_CHECK(cudaMemcpy(state->d_checkpoint_z,
                            state->d_checkpoint_scratch_z,
                            N_total * sizeof(float), cudaMemcpyDeviceToDevice));
      CUDA_CHECK(cudaMemcpy(state->d_checkpoint_mu_h,
                            state->d_checkpoint_scratch_mu_h,
                            N_total * sizeof(float), cudaMemcpyDeviceToDevice));
      CUDA_CHECK(cudaMemcpy(state->d_checkpoint_var_h,
                            state->d_checkpoint_scratch_var_h,
                            N_total * sizeof(float), cudaMemcpyDeviceToDevice));
      CUDA_CHECK(cudaMemcpy(state->d_checkpoint_log_w,
                            state->d_checkpoint_scratch_log_w,
                            N_total * sizeof(float), cudaMemcpyDeviceToDevice));
      CUDA_CHECK(
          cudaMemcpy(state->d_checkpoint_ll, state->d_checkpoint_scratch_ll,
...
#define DISPATCH_CPMMH(N)                                                      \
  kernel_cpmmh_rejuvenate_fused_impl<N>                                        \
      <<<state->N_theta, N, cpmmh_shared_mem_size<N>()>>>(                     \
          state->d_particles, state->d_particles_temp, state->d_y_history,     \
          curr_noise, other_noise, curr_u0, other_u0, state->t_current,        \
          state->N_theta, state->noise_capacity, state->cpmmh_rho,             \
          state->d_accepts, state->d_swap_flags, t_checkpoint_use, cp_z,       \
          cp_mu, cp_var, cp_logw, cp_ll)

    for (int k = 0; k < state->K_rejuv; k++) {
      int h_accepts = 0;
      CUDA_CHECK(cudaMemcpy(state->d_accepts, &h_accepts, sizeof(int),
                            cudaMemcpyHostToDevice));
      ...
      kernel_commit_accepted_noise<<<state->N_theta, state->N_inner>>>(
          curr_noise, other_noise, curr_u0, other_u0, state->d_swap_flags,
          state->N_theta, state->N_inner, state->t_current,
          state->noise_capacity, t_start_commit);
      /* Sync needed: we read d_accepts on host */
      CUDA_CHECK(cudaDeviceSynchronize());

      CUDA_CHECK(cudaMemcpy(&h_accepts, state->d_accepts, sizeof(int),
                            cudaMemcpyDeviceToHost));
...
#undef DISPATCH_CPMMH
    ...
    kernel_compute_outer_ess<<<1, ess_block, 32 * sizeof(float)>>>(
        state->d_particles, state->d_ess, state->N_theta);
    CUDA_CHECK(cudaDeviceSynchronize());
    CUDA_CHECK(cudaMemcpy(&h_ess, state->d_ess, sizeof(float),
                          cudaMemcpyDeviceToHost));
  }
  ...
  if (state->fixed_lag_L > 0) {
    ...
    if (target > state->t_checkpoint && state->t_current > 0) {
      kernel_save_checkpoint<<<state->N_theta, state->N_inner>>>(
          state->d_particles, state->d_checkpoint_z, state->d_checkpoint_mu_h,
          state->d_checkpoint_var_h, state->d_checkpoint_log_w,
          state->d_checkpoint_ll, state->N_theta, state->N_inner);
      CUDA_CHECK(cudaDeviceSynchronize());
...
static void smc2_cuda_get_theta_moments_internal(SMC2StateCUDA *state,
                                                 float *theta_mean,
                                                 float *theta_std) {
  ...
  CUDA_CHECK(cudaMemcpy(h_params[0], state->d_particles.rho,
                        state->N_theta * sizeof(float),
                        cudaMemcpyDeviceToHost));
  CUDA_CHECK(cudaMemcpy(h_params[1], state->d_particles.sigma_total,
                        state->N_theta * sizeof(float),
                        cudaMemcpyDeviceToHost));
  CUDA_CHECK(cudaMemcpy(h_params[2], state->d_particles.r_split,
                        state->N_theta * sizeof(float),
                        cudaMemcpyDeviceToHost));
  CUDA_CHECK(cudaMemcpy(h_params[3], state->d_particles.mu_base,
                        state->N_theta * sizeof(float),
                        cudaMemcpyDeviceToHost));
  CUDA_CHECK(cudaMemcpy(h_params[4], state->d_particles.mu_scale,
                        state->N_theta * sizeof(float),
                        cudaMemcpyDeviceToHost));
  CUDA_CHECK(cudaMemcpy(h_params[5], state->d_particles.mu_rate,
                        state->N_theta * sizeof(float),
                        cudaMemcpyDeviceToHost));
...
void smc2_cuda_get_theta_cov(SMC2StateCUDA *state, float *theta_mean,
                             float *theta_cov) {
  float *h_weight = (float *)malloc(state->N_theta * sizeof(float));
  float *h_params[N_PARAMS];
  for (int i = 0; i < N_PARAMS; i++)

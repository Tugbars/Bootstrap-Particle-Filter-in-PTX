 ...
 * Derived:
 *   sigma_z = r_split * sigma_total,  sigma_base = sqrt(1-r²) * sigma_total
 * Fixed:
 *   θ(z) speed curve (θ_base, θ_scale, θ_rate) in constant memory
 *
...
SMC2StateCUDA* smc2_cuda_alloc(int N_theta, int N_inner) {
    ...
    state->K_rejuv = 8;  /* 8D space needs more moves per rejuvenation */
    ...
    state->bounds.mu_scale_min = 0.01f;        state->bounds.mu_scale_max = 5.0f;
    state->bounds.mu_rate_min = 0.1f;          state->bounds.mu_rate_max = 10.0f;
    state->bounds.sigma_scale_min = 0.001f;    state->bounds.sigma_scale_max = 1.0f;
    ...
    state->ess_check_interval = 10;  /* Check ESS every 10 ticks in batch mode */
...
/*═══════════════════════════════════════════════════════════════════════════════
 * smc2_cuda_init_warm — Initialize from Kalman-predicted Gaussian
 *
 * Closes the loop: posterior from window k → prior for window k+1.
 *
 * 1. Sets prior means to warm_mean[i], stds to sqrt(warm_cov[i][i])
 * 2. Updates constant memory so CPMMH uses the warm prior (prevents ridge drift)
 * 3. Sets initial proposal Cholesky from warm_cov (better CPMMH starts)
 * 4. Calls existing init_from_prior kernel (samples N(mean_i, std_i²))
 *
 * The off-diagonal covariance is not used for particle initialization (sampling
 * is independent per parameter), but IS used for the initial CPMMH proposal.
 * The adaptive proposal will refine this from data within the window.
 *═══════════════════════════════════════════════════════════════════════════════*/

void smc2_cuda_init_warm(SMC2StateCUDA* state,
                          const float* warm_mean,
                          const float* warm_cov) {
    /* ── Step 1: Update prior struct with warm values ── */
    ...
    /* ── Step 2: Set initial proposal Cholesky from warm_cov ──
     * Cholesky of (2.38²/d) · warm_cov gives well-scaled CPMMH proposals.
     * This is what the adaptive proposal would converge to anyway. */
    float h_cov_scaled[N_PARAMS * N_PARAMS];
    float adaptive_scale = 2.38f * 2.38f / (float)N_PARAMS;
    for (int i = 0; i < N_PARAMS * N_PARAMS; i++)
        h_cov_scaled[i] = warm_cov[i] * adaptive_scale;

    /* Add small regularization */
    for (int i = 0; i < N_PARAMS; i++)
        h_cov_scaled[i * N_PARAMS + i] += 1e-8f;
    ...
    if (!chol_success) {
        /* Fallback: diagonal from warm_cov */
    ...
    /* Also update proposal_std for diagonal fallback path */
    for (int i = 0; i < N_PARAMS; i++)
        state->proposal_std[i] = sqrtf(fmaxf(warm_cov[i * N_PARAMS + i], 1e-8f));

    /* ── Step 3: Upload everything and call existing init ──
     * init_from_prior uploads prior, bounds, curve, proposal to constant memory,
     * re-seeds RNG, and calls kernel_init_from_prior. */
    CUDA_CHECK(cudaMemcpyToSymbol(d_prior, &state->prior, sizeof(SVPrior)));
    CUDA_CHECK(cudaMemcpyToSymbol(d_bounds, &state->bounds, sizeof(SVBounds)));
    CUDA_CHECK(cudaMemcpyToSymbol(d_theta_curve, &state->theta_curve, sizeof(SVCurve)));
    CUDA_CHECK(cudaMemcpyToSymbol(d_proposal_std, state->proposal_std, N_PARAMS * sizeof(float)));
...
/*═══════════════════════════════════════════════════════════════════════════════
 * smc2_cuda_update_batch — Process multiple observations with reduced sync
 *
 * Key optimizations over calling smc2_cuda_update() in a loop:
 *   1. Single bulk H→D transfer for all observations
 *   2. Deferred ESS check every ess_check_interval ticks
 *   3. Async ESS readback via pinned memory + stream
 *   4. Checkpoint sync only when checkpoint actually fires (every L ticks)
 *
 * On WDDM: saves ~375ms per 3000-tick window (eliminating ~2700 syncs)
 * On Linux/TCC: saves ~50ms (syncs are cheaper but still add up)
 *═══════════════════════════════════════════════════════════════════════════════*/
...
float smc2_cuda_update_batch(SMC2StateCUDA* state, const float* y_batch, int n_obs) {
    ...
    /* ── Step 1: Ensure y_history capacity ── */
    int needed = state->y_history_len + n_obs;
    if (needed > state->y_history_capacity) {
    ...
    /* ── Step 2: Bulk upload all observations ── */
    CUDA_CHECK(cudaMemcpy(&state->d_y_history[state->y_history_len],
                          y_batch, n_obs * sizeof(float),
                          cudaMemcpyHostToDevice));

    /* ── Step 3: ESS block size (computed once) ── */
    int ess_block = 1;
    while (ess_block < state->N_theta) ess_block *= 2;
    ess_block = (ess_block > 1024) ? 1024 : ess_block;

    float h_ess = (float)state->N_theta;  /* Assume healthy until proven otherwise */

    /* ── Step 4: Main tick loop ── */
    for (int i = 0; i < n_obs; i++) {
        float y_obs = y_batch[i];
        state->y_history_len++;
        state->t_current++;

        /* Noise capacity growth for L=0 mode (rare) */
        if (state->fixed_lag_L == 0 && state->t_current >= state->noise_capacity) {
            CUDA_CHECK(cudaStreamSynchronize(stream));
            int old_cap = state->noise_capacity;
            smc2_cuda_set_noise_capacity(state, state->noise_capacity * 2);
            if (state->noise_capacity == old_cap) break;  /* OOM — stop early */
        }

        /* Launch RBPF step on stream */
        #define DISPATCH_RBPF_BATCH(N) \
        ...
        switch (state->N_inner) {
            case 64:  DISPATCH_RBPF_BATCH(64);  break;
            case 128: DISPATCH_RBPF_BATCH(128); break;
            case 256: DISPATCH_RBPF_BATCH(256); break;
            case 512: DISPATCH_RBPF_BATCH(512); break;
            default: fprintf(stderr, "Unsupported N_inner=%d\n", state->N_inner); exit(EXIT_FAILURE);
        }
        #undef DISPATCH_RBPF_BATCH

        /* Launch ESS compute on stream */
        kernel_compute_outer_ess<<<1, ess_block, 32 * sizeof(float), stream>>>(
            state->d_particles, state->d_ess, state->N_theta);

        /* Save checkpoint if needed (periodic, every L ticks) */
        if (state->fixed_lag_L > 0) {
            int target = (state->t_current / state->fixed_lag_L) * state->fixed_lag_L;
            if (target > state->t_checkpoint && state->t_current > 0) {
        ...
        /* ── Deferred ESS check ──
         * Only sync and read ESS every `interval` ticks (or on the last tick).
         * Between checks, kernels pipeline freely on the GPU. */
        bool check_now = ((i + 1) % interval == 0) || (i == n_obs - 1);
        if (!check_now) continue;

        /* Async copy ESS to pinned host memory, then sync stream */
        ...
        for (int k = 0; k < state->K_rejuv; k++) {
            ...
            switch (state->N_inner) {
                ...
                case 128: DISPATCH_CPMMH_BATCH(128); break;
                case 256: DISPATCH_CPMMH_BATCH(256); break;
                case 512: DISPATCH_CPMMH_BATCH(512); break;
                default: fprintf(stderr, "Unsupported N_inner=%d\n", state->N_inner); exit(EXIT_FAILURE);
            }
...
/*═══════════════════════════════════════════════════════════════════════════════
 * Full 8×8 weighted posterior covariance — for Kalman parameter tracker
 *═══════════════════════════════════════════════════════════════════════════════*/
 ...
 *
 * z̄ = Σ_j w_j^outer · (1/N_inner) · Σ_i z_tilde_to_z(z̃_{i,j})
 *
 * Inner weights are approximately uniform after resampling, so we use
 * the unweighted inner mean for simplicity. The outer weights carry
 * the main information about which θ-particles (and their z clouds)
 * are plausible.
 *═══════════════════════════════════════════════════════════════════════════════*/
...
float smc2_cuda_get_z_mean(SMC2StateCUDA* state) {
    ...
    for (int j = 0; j < state->N_theta; j++) {
        ...
        for (int i = 0; i < state->N_inner; i++) {
            /* z̃ is stored as z̃, need to map to z via tanh squash */
            float zt = h_z[base + i];

...
1. **Phase thresholds need tuning.** Current thresholds are too permissive — the DGP's z process spans [0, 3] even in "calm" segments (z_max = 2.96 during Calm warmup). Phase 1→2 fires at tick 4500 and Phase 2→3 at tick 6000, meaning Phased runs as effectively-All8-with-good-init for 95% of the test. Raise `ceiling_z_threshold` to 4.0+ or increase `ceiling_z_sustained` to 10+ windows so Phase 2 triggers only on genuine crises. This would produce a cleaner Phase 1 = Fixed4 baseline in early segments and confirm that Phase 2 kicks in *because of* stress, not because OU wanders to z = 2.96 during calm.

2. **DGP z-range is suspicious.** Every segment — including "Calm warmup" — shows z_min near 0 and z_max near 3.0. The OU process for z explores the full range regardless of segment label. Either mean-reversion is too low (so z diffuses widely even when σ is small), or the z_bias mechanism isn't creating distinct regimes. Worth checking whether the DGP actually produces differentiated calm vs crisis z-distributions, or just OU noise that always fills [0, 3].

3. **`gpu_bpf_set_sigma_z()` doesn't exist.** The BPF API has no setter for σ_z — it's set at creation time only. The pipeline currently cannot push SMC²'s σ_z posterior to the dBPF. In practice σ_z changes slowly, so this is low priority, but a proper setter (or BPF re-creation at epoch boundaries) would close the loop. Tracked as a TODO in `smc2_bpf_pipeline.h`.

4. **ASYNC mode untested.** The pipeline supports async mode (dBPF never blocks, SMC² pushes when ready), but all current tests use SYNC mode for deterministic validation. Need to verify that async param pushes don't corrupt dBPF state mid-kernel. The design uses `cudaStreamQuery` polling so there's no race on the GPU side, but the host-side `gpu_bpf_set_mu()` write needs to happen between dBPF kernel launches, not during.

5. **HRBPF integration.** The next architectural upgrade: replace the bootstrap inner filter in SMC² with a Hierarchical Rao-Blackwellized Particle Filter. HRBPF analytically marginalizes `h` given `z̃`, requiring exponentially fewer particles for the same likelihood accuracy. This would make CPMMH acceptance rates viable for the full 8-param space and is the path to true online 8-param learning without phased gating.

6. **Paper update.** The arxiv draft (`fl_cpmmh_smc2_paper.pdf`, currently 10 pages) needs a new section on phased learning with the gauntlet results. The floor/ceiling reparameterization and sequential identification story is the strongest methodological contribution — more novel than the CPMMH mechanics.
